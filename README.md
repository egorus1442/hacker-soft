...
Записи в отчете разделены на три части, чтобы счетчик находок не смешивал
проблемы с фактами и статусом сканера:

- `Проблемы к исправлению` - то, с чем нужно что-то делать;
- `Инвентарь и контекст` - собранные факты о поверхности (технологии, порты, документы);
- `Диагностика сбора` - как прошел сам скан и что осталось непроверенным.

Однотипные проблемы на множестве хостов объединяются в одну запись со списком
целей, поэтому «нет security headers» больше не превращается в десяток карточек.
...
Дорки, которые вообще не дошли до поисковика из-за капчи, блокировки или
таймаута, попадают в отдельный раздел `Непроверенные дорки` вместе со ссылками
для ручной проверки. Это важное отличие: «проверено и пусто» и «проверить не
удалось» больше не выглядят одинаково.
...
Без набора шаблонов `nuclei` не выполняет ни одной проверки и завершается с
`no templates provided for scan`, поэтому шаблоны нужны обязательно.

Docker-образ выкачивает их на этапе сборки в `/opt/nuclei-templates`, а
`docker-compose.yml` монтирует этот каталог named volume `nuclei-templates`,
чтобы обновления сохранялись между запусками.
...
```bash
export HACKER_SOFT_NUCLEI_TEMPLATES=/opt/nuclei-templates
```
...
Команда показывает `НЕ ГОТОВ` для установленного `nuclei` без шаблонов. Если
шаблонов нет, модуль не запускает сканирование вслепую, а пишет в отчет, что
проверки уязвимостей не выполнялись.
...
- Проверка устаревших `TLS 1.0/1.1` делает отдельный handshake на каждую версию.
  Если локальный OpenSSL собран без поддержки старых протоколов, результат
  помечается как непроверенный, а не как «чисто».
- Открытый рискованный порт дополнительно сверяется с reverse DNS. Если IP
  принадлежит внешнему хостингу, уверенность находки снижается: сервис может
  относиться к площадке провайдера, а не к вашей инфраструктуре.
